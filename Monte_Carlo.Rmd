...
Emissions <- Emissions %>% 
  mutate(Xcurt = ifelse(Xdelta<curt,ifelse(curt == 0,0,Xdelta),curt)) %>% #sum and run percentages on Xcurt 
  mutate(Xextra = ifelse(Xdelta>curt, ifelse(Xcurt-Xdelta< 0,0, Xcurt-Xdelta),0))
...
#Calculate sum of curtailment
#sum Xcurt. divide by curt. = proportion of curtailment that could be addressed then multiply that by Curtailmentcost
Xcurt_i_h <- sum(Emissions$Xcurt[intervention_emission_hours])
Xcurt_o_h <- sum(Emissions$Xcurt[-intervention_emission_hours])
Xcurtpotential <- sum(Xcurt_i_h,Xcurt_o_h)*0.75 #eek. this 75% is weird and doesn't match the solar fuel mix %. we need another column for solar reduction potential in the fuel mix. 
Xcurtextra <- sum(Emissions$Xextra) #check sums correctly)
Xcurtproportion <- Xcurtpotential/sum(Emissions$curt)

Curtailmentcost <- Xcurtpotential*0.75*-Curtailmentcost
#add to output table
Emissions_Table <- Emissions_Table %>% 
   mutate(ChangeCurtCost = c("NA", "NA", "NA", Curtailmentcost))
...
Nov_28_18_DC_Event_Usage <- filter(event_data, Date == "2018-11-28" & segment == "Workplace") %>%
select(Demand) %>%
unlist()
