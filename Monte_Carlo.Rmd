...
hourly_demand <- function(schedule = sch, throttle_schedule = tsch,price_change = p_c,intervention_hours = i_h,throttle_amount = t_a,throttle_hours = t_h){
...
```{r explore}
library(ggplot2)
freq_graph <- ggplot(sim_result, aes(x = Xt))+
  geom_histogram(aes(fill = Hr)) +
  facet_wrap(~Hr, scales = "free")
