...
simulation <- function(simulations = 100, 
  ...
  for(i in seq(simulations)){
    run_i <- hourly_demand(method = method_draws[i],
                       ...
                       intervention_comm_effect = sim_intervention_comm_effect)
    ...
    summarise_at(vars(Xf, P0, Xi, X0, P1, X1, Xint_effect, Xf, I01),funs(mean,min,max))
...
emissions_fcn <- function(EVDemand, peak_hours = pk, year = yr) {
...
#Load emissiosn factors  based on year
  ...
  if(year == 2018) {
   ...
   Hourly_EF <- read_csv("Hrly_EF_2025.csv")
  }
...
Emissions <- data.frame(Hr = c(1:24)) %>% 
  ...
  mutate(I01 = EVDemand$sim_result_EV_Demand$I01_mean) %>% 
  mutate(Xi = EVDemand$sim_result_EV_Demand$Xi_mean) %>% # column for initial demand at each hour not scaled
  mutate(X0 = EVDemand$sim_result_EV_Demand$X0_mean) %>% # column for initial demand at each hour scaled by chargers
  mutate(Xf = EVDemand$sim_result_EV_Demand$Xf_mean) %>% 
  mutate(Xdelta = EVDemand$sim_result_EV_Demand$Xint_effect_mean) %>% 
     # column for new demand at each hour. we'll need to update "Xt" to reflect the new column post comms/all interventions
  mutate(Pbase = EVDemand$sim_result_EV_Demand$P0_mean*EVDemand$sim_result_EV_Demand$X0_mean) %>% #total price paid in that hour ($/kwh * kwh)
  mutate(Pnew = EVDemand$sim_result_EV_Demand$P1_mean*EVDemand$sim_result_EV_Demand$Xf_mean) #total price paid in that hour ($/kwh * kwh)
