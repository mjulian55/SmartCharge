...
baseline <- read_csv("Model_Map/DC_Avg_Usage.csv")
...
hourly_demand <- function(segment = sg, month = mth, charger_power = pwr, schedule = sch, throttle_schedule = tsch,price_change = p_c,intervention_hours = i_h, intervention_chargers = int_ch, int_equals_baseline = int_e_b, throttle_amount = t_a,throttle_hours = t_h){
...
baseline_month<-unlist(select(baseline, month))
...
#EV_Demand$comm_shift[intervention_hours] <- (EV_Demand$Xint_effect[intervention_hours]/Change_intervention)*comm_shifting
#EV_Demand$comm_shift[-intervention_hours] <- (Xint_effect/Change_outside_intervention)*-comm_shifting



#EV_Demand <-   mutate(EV_Demand, XChangeSDGE = Xt - X0) %>% 
  #mutate(XChangeNoComm = XChangeSDGE/(1+SDGE_comm_effect)) %>% 
  #mutate(XChangeComm = XChangeNoComm*(1+intervention_comm_effect)) %>% 
  #mutate(Xf = XChangeComm)
...
#Load default periods (intervention hour is set above)
pk <- c(17:21) #target window to shift out off (this is only used in the output calculations below, not for the function)
o_h <- c(1:24) #create 1:24 for all other hours
o_h <-o_h[!o_h %in% i_h] #remove the intervention hours
o_h <-o_h[!o_h %in% pk] #remove the pk hours
#NOTE THIS NEEDS TO BE CHANGED TO REFLECT THE INTERVENTION HOURS FROM THE PREVIOUS FUNCTION... Currently set to filter based on i_h and pk. But i_h needs to be based on what's coming out of the "intervention_hours" in the first function. This needs to be moved into the function below. 
...
emissions_fcn <- function(EVDemand, intervention_emission_hours = i_h, peak_hours = pk, other_hours = o_h) {
...
Emissions <- Hourly_EF %>% 
    ...
    mutate(Xf = EVDemand$EV_Demand$Xt) # column for new demand at each hour. we'll need to update "Xt" to reflect the new column post comms/all interventions
return(Emissions)
  
}

Emissions <- emissions_fcn(EVDemand = hourly_demand())

#
Emissions <- Hourly_EF %>% 
  mutate(Xi = EV_Demand$EV_Demand$Xi) %>% # column for initial demand at each hour not scaled
  mutate(X0 = EV_Demand_run1$EV_Demand$X0) %>% # column for initial demand at each hour scaled by chargers
  mutate(Xf = EV_Demand_run1$EV_Demand$Xt) # column for new demand at each hour. we'll need to update "Xt" to reflect the new column post comms/all interventions
Emissions <- Emissions %>% 
...
#Sum base emissions by period (intervention window, peak window, other)
 CO2X0_i_h <- sum(Emissions$CO2X0[i_h]) #11-3 window
 CO2X0_pk <- sum(Emissions$CO2X0[pk]) #peak 4pm-9pm window
 CO2X0_o_h <- sum(Emissions$CO2X0[o_h]) #all other hours
 CO2X0_sum <- sum(Emissions$CO2X0)
 NOXX0_i_h <- sum(Emissions$NOXX0[i_h]) #11-3 window
 NOXX0_pk <- sum(Emissions$NOXX0[pk]) #peak 4pm-9pm window
 NOXX0_o_h <- sum(Emissions$NOXX0[o_h]) #all other hours
 NOXX0_sum <- sum(Emissions$NOXX0)
 
#Sum final emissions by period (intervention window, peak window, other)
 CO2Xf_i_h <- sum(Emissions$CO2Xf[i_h]) #11-3 window
 CO2Xf_pk <- sum(Emissions$CO2Xf[pk]) #peak 4pm-9pm window
 CO2Xf_o_h <- sum(Emissions$CO2Xf[o_h]) #all other hours
 CO2Xf_sum <- sum(Emissions$CO2Xf)
 NOXXf_i_h <- sum(Emissions$NOXXf[i_h]) #11-3 window
 NOXXf_pk <- sum(Emissions$NOXXf[pk]) #peak 4pm-9pm window
 NOXXf_o_h <- sum(Emissions$NOXXf[o_h]) #all other hours
 NOXXf_sum <- sum(Emissions$NOXXf)
 
#Sum change in emissions by period (intervention window, peak window, other)
 CO2Xdelta_i_h <- sum(Emissions$CO2Xdelta[i_h]) #11-3 window
 CO2Xdelta_pk <- sum(Emissions$CO2Xdelta[pk]) #peak 4pm-9pm window
 CO2Xdelta_o_h <- sum(Emissions$CO2Xdelta[o_h]) #all other hours
 CO2Xdelta_sum <- sum(Emissions$CO2Xdelta)
 NOXXdelta_i_h <- sum(Emissions$NOXXdelta[i_h]) #11-3 window
 NOXXdelta_pk <- sum(Emissions$NOXXdelta[pk]) #peak 4pm-9pm window
 NOXXdelta_o_h <- sum(Emissions$NOXXdelta[o_h]) #all other hours
...
#curtailment cost reduction: to be added
return(Emissions_Table)
}

#emissionsrun_1 <- emissions_fcn()
```
...
sim_result <- hourly_demand() %>% 
  mutate(i_draw = intervention_draws[1], t_draw = throttle_draws[1])
sim_result <- sim_result[0,]


for(i in seq(simulations)){
  run_i <- hourly_demand(schedule = intervention_draws[i],throttle_schedule = throttle_draws[i],throttle_amount = 0) %>% 
    mutate(i_draw = intervention_draws[i], t_draw = throttle_draws[i])
  
  sim_result <- rbind(sim_result,run_i)
...
library(ggplot2)
freq_graph <- ggplot(sim_result, aes(x = Hr, y = Xt))+
 geom_jitter(aes(color = Xt), alpha = 0.5) +
  theme_minimal() +
...
graph_table <- EV_Demand_run1[c(1,5,14)] %>% 
  gather(condition,value,X0:Xt) %>% 
  mutate(Theoretical_Max=EV_Demand_run1$MT[1])
