...
price_change_no_comm <- price_change/(1+SDGE_commm_effect)
price_change_int_comm <- price_change_no_comm*(1+intervention_comm_effect)
...
P1_No_C <- EV_Demand$P1
P1_No_C[intervention_hours] <-  P1_No_C[intervention_hours]/(1+SDGE_commm_effect)

P1C <-P1_No_C
P1C[intervention_hours] <- P1C[intervention_hours]*(1+intervention_comm_effect)

#Adds percentage change in price (P1p)
EV_Demand <- EV_Demand %>% 
  mutate(P1_No_C = P1_No_C) %>% 
  mutate(P1C = P1C) %>% 
  mutate(P1p = (P1C-P0)/P0)
...
EV_Demand_run1 <- hourly_demand(month = "Nov_18",SDGE_commm_effect = 0.2,intervention_comm_effect = 0, throttle_amount = -0.5, price_change = -0.05, intervention_hours = c(12:15))
...
simulation <- function(simulations = 100, 
                       ...
                       sim_SDGE_commm_effect = SDGE_c_e, 
  ...
  for(i in seq(simulations)){
    run_i <- hourly_demand(segment = sim_segment, 
                          ...
                          SDGE_commm_effect = sim_SDGE_commm_effect, 
...
EV_Demand_run1 <- hourly_demand(segment = "Workplace", month = "June_18",SDGE_commm_effect = 0,intervention_comm_effect = 0, throttle_amount = 0,throttle_hours = c(6:11), price_change = -0.05, intervention_hours = c(12:15))
