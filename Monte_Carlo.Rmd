...
EV_Demand <- mutate(price_schedule, Xi = baseline_month, X0 = baseline_month/baseline_chargers*intervention_chargers) #340 here comes from the number of chargers installed for the baseline
...
emissions_fcn <- function(intervention_emission_hours = i_h, peak_hours = pk, other_hours = o_h) {

#Create Date Frame for Emissions Outputs
Emissions <- Hourly_EF %>% 
    mutate(Xi = EV_Demand_run1$EV_Demand$Xi) %>% # column for initial demand at each hour not scaled
