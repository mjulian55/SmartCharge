...
baseline <- read_csv("Model_Map/Workplace_Avg_Usage.csv")
...
summary_test <- group_by(sim_result_EV_Demand, Hr) %>% 
...
library(ggplot2)
freq_graph <- ggplot(sim_result_EV_Demand, aes(x = Hr, y = X0))+
  geom_line(aes(x = Hr, y = X0), alpha = 0.5, fill = "blue")+
  geom_jitter(aes(y = Xf, color = Xf>X0), alpha = 0.5) +
