...
sch <- 3 #elasticities to use for price intervention (column in the elasticities dataframe) -  Non PV Summer Weekday EPEV L
tsch <- 3 #elasticites to use for throttling intervention (column in the elasticities dataframe) - Non PV Summer Weekday EPEV L
...
EV_Demand <- mutate(price_schedule, Xi = baseline$Jul_18, X0 = baseline$Jul_18/baseline_chargers*intervention_chargers) #340 here comes from the number of chargers installed for the baseline
...
freq_graph <- ggplot(sim_result, aes(x = Hr, y = Xt))+
 geom_jitter(aes(color = Hr), alpha = 0.5) +
  theme_minimal()
