...
sch <- 3 #elasticities to use for price intervention (column in the elasticities dataframe)
tsch <- 3 #elasticites to use for throttling intervention (column in the elasticities dataframe)
...
hourly_demand <- function(segment = sg, month = mth, charger_power = pwr, schedule = sch, throttle_schedule = tsch,price_change = p_c,intervention_hours = i_h,throttle_amount = t_a,throttle_hours = t_h){
...
number_chargers <-filter(Chargers, Market_Segment == segment) %>% 
  select(month) %>% 
  as.numeric()

#WP_Chargers <- chargers$Workplace #Number of Chargers (C)
EV_Demand <- mutate(price_schedule, Xi = baseline$Xi, X0 = baseline$Xi/baseline$C[1]*number_chargers) #340 here comes from the number of chargers installed for the baseline
...
Max_Theory <- number_chargers*charger_power
