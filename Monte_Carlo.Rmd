...
o_h <- c(1:11,16,22:24) #all other hours outside the target peak window and intervention hours (again, for output calculations below, not for the function)
...
# Elasticity 
chosen_elasticities <- Elasticities[c(1,2,3)] #this pulls out columns 1, 2, and the designated elasticity (from row 74 into a new dataframe) CHANGE 3 BACK TO schedule
...
EV_Demand <- mutate(price_schedule, Xi = baseline$Jul_18, X0 = baseline$Jul_18/baseline_chargers*intervention_chargers) #340 here comes from the number of chargers installed for the baseline. This needs to be fixed so that we're actually selecting the month and not hard coding.  
...
Tp[throttle_hours] <- throttle_amount #Assigns the throttling intervention percentage chosen at the inputs at the beginning tho the hours chosen then
...
EV_Demand_run1 <- hourly_demand() #creates a data frame with the output from the function, which is needed to find the emission factors.
...
Emissions <- Hourly_EF %>% 
  mutate(Xi = EV_Demand_run1$Xi) %>% # column for initial demand at each hour not scaled
  mutate(X0 = EV_Demand_run1$X0) %>% # column for initial demand at each hour scaled by chargers
  mutate(Xf = EV_Demand_run1$Xt) # column for new demand at each hour. we'll need to update "Xt" to reflect the new column post comms/all interventions
Emissions <- Emissions %>% 
...
graph_table <- HD[c(1,5,14)] %>% 
  gather(condition,value,X0:Xt) %>% 
  mutate(Theoretical_Max=HD$MT[1])
