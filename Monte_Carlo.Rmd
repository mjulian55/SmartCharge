...
#PF1S = 2019 TOU + discount + air pollution & price comm + morning throttling + 2030 Chargers
FW1S <- simulation(simulations = 1000, 
                   sim_seg = "Workplace",
                       sim_month = 7, 
                       sim_year = 2018,
                      sim_intervention_chargers = 31435, 
...
write_csv(FW1S$sim_result_summary, "Results/Workplace_FW1S_Demand_Summary.csv")

#run emissions and write to csv
FW1S_output_table <- emissions_fcn(PF1S) 
write_csv(FW1S_output_table$Emissions_Table, "Results/Workplace_FW1S_Output_Summary.csv") #write emissions to csv
write_csv(FW1S_output_table$Emissions, "Results/Workplace_FW1S_Output_All_Hours.csv")
...
#FW1S = 2019 TOU + discount + air pollution & price comm + morning throttling + 2030 Chargers
FW1S_demand_summary <- read_csv("Results/Workplace_FW1S_Demand_Summary.csv") 

graph_tableFW1S <- FW1S_demand_summary[c("Hr","X0_mean","Xf_mean")] %>% 
  gather(condition,value,c("X0_mean","Xf_mean")) 
#%>% mutate(Theoretical_Max=FW1S_demand_summary$MT[1])

Demand_GraphFW1S <- ggplot(data = graph_tableFW1S, aes(x = Hr)) +
  geom_line(aes(y = value, color=condition)) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
  panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  labs(title="Workplaces (FW1S) (TOU 2019, discount, air & price comm, throttling) Hourly Demand Forecast",
...
#Demand_GraphFW1S
ggsave("Results Graphs/Workplace_FW1S_Demand_Summary.pdf", Demand_GraphFW1S, device = "pdf", width = 8, height = 4.5)
