...
Emissions <- Hourly_EF %>% 
  ...
  mutate(Xf = EVDemand$EV_Demand$Xt) 
...
#add to output table
Emissions_Table <- Emissions_Table %>% 
   mutate(CurtCostRed = c(Curtailmentcost_i_h, "NA", "NA", Curtailmentcost_i_h))

return(list(Emissions=Emissions, Emissions_Table=Emissions_Table))
  
}
Emissions_run1 <- emissions_fcn(EVDemand = hourly_demand()) #or run as emissions_fcn(EV_Demand_run1)
