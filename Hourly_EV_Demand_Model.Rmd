...
for(i in x) {
    nam <- paste("X", i, sep = ".")
    periods <- price_schedule$period[c(sort(c(i,3.5,10,15,20.5)),i)]
    own_period <- price_schedule$period[i]
    elastic <- Elasticities[own_period,periods] 
    assign(nam,data.frame(Hour=sort(c(i,3.5,10,15,20.5,i+24)),Period=price_schedule$period[c(sort(c(i,3.5,10,15,20.5)),i)],Elasticity = as.numeric(elastic)))
