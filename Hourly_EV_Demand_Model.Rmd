...
for(i in x) {
  ...
  Hrs <- sort(c(i,10,15,20.5))
    periods <- price_schedule$period[c(Hrs,i)]
    own_period <- price_schedule$period[i]
    elastic <- Elasticities[own_period,periods] 
    assign(nam,data.frame(Hour=c(Hrs,i+24),Period=periods,Elasticity = as.numeric(elastic)))
    
}

for (i in x) {
  current_hr <- eval(parse(text = sub("XX", i, "X.XX")))
  current_hr <- mutate(current_hr, Hour24 = ifelse(Hour<=24,Hour,Hour-24))
