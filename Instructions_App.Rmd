...
That's a great question!  Follow this flow chart to understand how the wdigets work, what some of the inputs are, and how you can change them in order to create some outputs. The widgets are on the left in the **sidebar.**


### **Choose your Widgets in the Sidebar**


#### **First, choose how many chargers you want in your simulation**
...
1) Not at all, where no pricing intervention is selelcted.  
...
#### **Apply a Throttling Rate to the electric vehicle chargers if desired**

Throttling is when the power draw of the electric vehicle charger is manually reduced.  For example, if a 6.6 kilowatt (kW) rated EV charger charges an EV for an hour, that EV would have used 6.6 kilowatt *hours* (kWh).  But if 50% throttling was applied to that charger, only 3.3 kWh of electricty would have been used.
...
There are 3 Outputs that appear in the right main panel based on the inputs you choose from the left sidebar panel.

1) The top graph is the simulated demand (the green line) compared to the baseline demand that you start with (the red line).  Only the red line appears at first until you start choosing inputs.
Shaded areas will appear for the time periods of any intervention you choose.

2) The table under this demand graph displays various economic and greenhouse gas emission changes that occur as a result of your simulation.

3) The bottom graph is similar to the top graph, only it takes the same model used to create the first graph and runs it many times (this amount can be chosen by you).  This process is called a Monte Carlo simulation.  It's used to randomly use the same model to repeatedly simulate your results so any uncertainty and range can be displayed.  There are 4 methods being used within our computer model to simulate demand, and as each one is randomly drawn, you can see which one is being used and how that one is displayed.
